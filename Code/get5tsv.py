...
ans['upvotebytime'] = upvotebytime
ans = ans[['owner', 'question_id', 'upvotebytime']]

questions.columns = ['asker', 'question_id']
ans.columns = ['answerer', 'question_id', 'upvotebytime']
...
for i, row in df.iterrows():
    if row['asker'] not in best_ans or row['upvotebytime'] > best_ans[row['asker']][1]:
        best_ans[row['asker']] = [row['answerer'], row['upvotebytime']]
...
for q in qs:
    ...
    if len(rows) > 1:
        besta = rows[rows['upvotebytime'] == max(rows['upvotebytime'])]
        if besta['upvotebytime'].values[0] > 0:
            bestv = besta['upvotebytime'].values[0]
            ...
            for i, row in rows.iterrows():
                if row['answerer'] != besta:
                    ...
                    vben['to'].append(besta)
                    vben['weight'].append(bestv - row['upvotebytime'])
                    
                    vben2['from'].append(row['answerer'])
                    vben2['to'].append(besta)
                    vben2['weight'].append(bestv - row['upvotebytime'])
                    vben2['from'].append(row['asker'])
                    vben2['to'].append(row['answerer'])
                    vben2['weight'].append(bestv - row['upvotebytime'])
